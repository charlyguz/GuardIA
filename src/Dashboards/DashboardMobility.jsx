...
const DashboardMobility = () => {
  ...
  const handleNotification = (probabilities) => {
    const truncatedProbabilities = probabilities.map((prob) => prob.toFixed(2));
    notification.warning({
      ...
      description: (
        <span style={{ color: "black" }}>
          Se ha encontrado un arma con una confianza de :{" "}
          {truncatedProbabilities.join(", ")}
        </span>
      ),
      style: {
    ...
    <div className="flex">
      <div className="relative w-1/5 h-screen bg-opacity-100 overflow-hidden">
        ...
        <nav className="relative z-10 px-4 mt-6">
          <h2 className="text-4xl font-extrabold text-white my-2 h-28">
            GuardIA Mobility
          </h2>
          <ul className="space-y-4 text-2xl">
            ...
            >
              <CameraIcon
                className={`h-10 w-10 ${
                  selected === "Cámaras" ? "text-black" : "text-white"
                }`}
              />
              <span>Cámaras</span>
            </li>
            ...
            >
              <MegaphoneIcon
                className={`h-10 w-10 ${
                  selected === "SOS" ? "text-black rounded-lg" : "text-white"
                }`}
              />
              <span>SOS</span>
            </li>
            ...
            >
              <CpuChipIcon
                className={`h-10 w-10 ${
                  selected === "IA" ? "text-black rounded-lg" : "text-white"
                }`}
              />
              <span>IA</span>
            </li>
      ...
      <div className="flex-grow p-8">
        ...
        <div className="flex-grow p-8">
          <Camera onDetection={handleNotification} />
        </div>
      </div>
    </div>
  );
